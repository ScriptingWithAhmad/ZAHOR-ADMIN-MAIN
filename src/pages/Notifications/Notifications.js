...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    axios.get(apiUrl + "adminNotification/getAll").then((res) => {
      console.log(res)
      setCat(res.data)
  ...
  const fireAlert = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(apiUrl + "adminNotification/delteByID?id=" + id).then((resp) => {
            var requestOptions = {
                method: 'GET',
                redirect: 'follow'
              };
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <Link to="/Add_Notifications" className="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
          ...
          {cat?<tbody>
            {cat.map((data, index) => {
              return <tr>
                ...
                {category ? <td className="py-3">{data?.title}</td> : null}
                ...
                <td className="py-3">
                  <small
                    ...
                    onClick={()=>{navigate("/Add_Notifications/"+data._id)}}
                  >
                    Edit
                  </small>
                  <small
